...
export interface SettingsDefaults {
  ...
  CLAUDE_MEM_OPENROUTER_QA_MODEL: string;     // Knowledge-agent Q&A model (corpus query). Accepts a concrete id or a $TIER:* alias; empty = use CLAUDE_MEM_OPENROUTER_MODEL.
...
export class SettingsDefaultsManager {
  private static readonly DEFAULTS: SettingsDefaults = {
    ...
    CLAUDE_MEM_OPENROUTER_MODEL: 'xiaomi/mimo-v2-flash:free',  // Default OpenRouter model (free tier) — bulk observation/summary generation
    CLAUDE_MEM_OPENROUTER_QA_MODEL: '',  // Knowledge-agent Q&A model; empty = reuse CLAUDE_MEM_OPENROUTER_MODEL. Set a stronger model (or $TIER:smart) for corpus answers.
