...
| `CLAUDE_MEM_PROJECT_ROOT`     | _(unset)_                       | A non-empty environment value hard-overrides hook cwd/Git/worktree project identity for all memory reads and writes |

### Shared Project Root

Set `CLAUDE_MEM_PROJECT_ROOT` in the environment that launches the CLI:

```bash
export CLAUDE_MEM_PROJECT_ROOT=/Users/belokobylskiiilia/Development/MKS
```

When set to a non-empty value, it takes precedence over the hook cwd, Git root, and worktree identity for every memory read and write. Do not put it in `settings.json`.

The shared project key is derived through the existing `getProjectName` behavior: the configured root's Git repository name when available, otherwise its path basename. In this example, `MKS` becomes the shared project key.
