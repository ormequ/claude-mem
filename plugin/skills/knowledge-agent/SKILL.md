...
**Both `build_corpus` and `query_corpus` use an English-only embedding model.**
A non-English query lands in a degenerate region where texts are similar by
script rather than by meaning — it returns confident, plausible-looking results
that have nothing to do with the question, and misses English records even when
they are exactly on target.

- `build_corpus(query=…)` — the semantic filter must be in English. Keep
  identifiers (service names, file paths, ticket ids, error strings) verbatim;
  translate only the prose around them.
- `query_corpus(question=…)` — same rule for the Q&A question.
- Other languages are fine for specific literal matches (a Russian error message,
  a file path) — just don't use them for the semantic part of the query.
