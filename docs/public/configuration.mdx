...
```bash
export CLAUDE_MEM_PROJECT_ROOT=/Users/you/Development/ACME
```
...
The shared project key is derived through the existing `getProjectName` behavior: the configured root's Git repository name when available, otherwise its path basename. In this example, `ACME` becomes the shared project key.
