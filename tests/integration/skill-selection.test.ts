...
describe('Skill selection', () => {
  ...
  it('compact set is knowledge-agent + mem-search + pathfinder + timeline-report + harness-review', () => {
  ...
  // scripts/sync-marketplace.cjs hand-duplicates both lists (it is .cjs, this is TS with no
  // per-file build artifact to require). That duplication drifted silently once already —
  // harness-review was added here and never reached the script, so it was absent from every
  // install while the source said it shipped. Assert the copies match instead of trusting a
  // "keep in sync" comment.
  it('sync-marketplace.cjs carries the same skill sets as this module', () => {
    const source = readFileSync(join(import.meta.dir, '../../scripts/sync-marketplace.cjs'), 'utf-8');

    const parseList = (name: string): string[] => {
      const match = source.match(new RegExp(`const ${name} = \\[([^\\]]*)\\]`));
      if (!match) throw new Error(`${name} not found in sync-marketplace.cjs`);
      return [...match[1].matchAll(/'([^']+)'/g)].map(m => m[1]);
    };

    expect(parseList('DEFAULT_CLAUDE_MEM_SKILLS')).toEqual([...DEFAULT_CLAUDE_MEM_SKILLS]);
    expect(parseList('COMPACT_CLAUDE_MEM_SKILLS')).toEqual([...COMPACT_CLAUDE_MEM_SKILLS]);
  });
  ...
  it('validates plural unknown extras before removing default-set exclusions', () => {
    ...
    expect(() => filterClaudeMemSkillsDirectory(skillsDir)).toThrow(
      'Unknown Claude-Mem extra skills: babisit, not-real. Available bundled skills: harness-review, how-it-works, knowledge-agent, learn-codebase, mem-search, pathfinder, smart-explore, standup, timeline-report, weekly-digests, wowerpoint',
    );
