...
export function queryObservationsMulti(
  ...
  return db.db.prepare(`
    ...
    WHERE (o.project IN (${projectPlaceholders})
      ...
      -- fork: an empty concepts array passes. Emitting no concept is a valid and
      -- deliberate outcome of the sparse-tagging prompt, but this gate turned it into
      -- permanent invisibility. See FORK_NOTES "Empty concepts pass the injection gate".
      AND (
        o.concepts IS NULL
        OR trim(o.concepts) IN ('', '[]')
        OR EXISTS (
...
export function countObservationsByProjects(db: DatabaseOwner, projects: string[]): number {
