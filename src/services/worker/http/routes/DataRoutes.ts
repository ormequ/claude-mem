...
// fork: search results truncate prompt_text to ~60 chars, and there was no way to read a
// prompt in full. See FORK_NOTES "Full prompt text by id".
const promptsBatchSchema = z.object({
  ids: integerArrayLike,
  orderBy: z.enum(['date_desc', 'date_asc']).optional(),
  limit: z.number().int().positive().optional(),
  project: z.string().optional(),
...
export class DataRoutes extends BaseRouteHandler {
  ...
  setupRoutes(app: express.Application): void {
    ...
    app.post('/api/prompts/batch', validateBody(promptsBatchSchema), this.handleGetPromptsByIds.bind(this));
  ...
  // fork: batch sibling of /api/prompt/:id. Returns prompt_text in full, unlike search.
  private handleGetPromptsByIds = this.wrapHandler((req: Request, res: Response): void => {
    const { ids, orderBy, limit, project } = req.body as z.infer<typeof promptsBatchSchema>;
