...
describe('MCP runtime-aware tool visibility', () => {
  ...
  it('tools/list path references the helper so discovery is runtime-aware', () => {
    ...
    // FORK: the helper is fed `enabledTools`, not `tools` — CLAUDE_MEM_SMART_TOOLS=false
    // must drop the smart_* tools from ListTools too (FORK_NOTES.md).
    expect(mcpServerSrc).toContain('getAdvertisedMcpToolsForRuntime(enabledTools, selectRuntime())');
