...
describe('search merged_into_project (adopt) scoping', () => {
  ...
  beforeEach(() => {
    ...
    seedObservation('wt-sess', 'wt-mem', 'acme-app/acme-app', 'Worktree finding', 'shared adoption keyword from worktree');
    // Native parent-repo row.
    seedObservation('parent-sess', 'parent-mem', 'acme-app', 'Parent finding', 'shared adoption keyword from parent');
    // Unrelated foreign project: must never leak into a acme-app-scoped search.
    seedObservation('other-sess', 'other-mem', 'wp-rosreestr-plugin', 'Foreign finding', 'shared adoption keyword from foreign');

    adopt('acme-app/acme-app', 'acme-app');
  });
  ...
  it('includes adopted worktree rows when scoped to the parent project (FTS path)', () => {
    const results = search.searchObservations('adoption', { project: 'acme-app' });
  ...
  it('includes adopted worktree rows on the no-query (filter-only) path too', () => {
    const results = search.searchObservations(undefined, { project: 'acme-app' });
    const titles = results.map(r => r.title).sort();
    expect(titles).toEqual(['Parent finding', 'Worktree finding']);
  });
  ...
  it('still matches the composite project key directly (provenance preserved)', () => {
    const results = search.searchObservations('adoption', { project: 'acme-app/acme-app' });
  ...
  it('getObservationsByIds hydrates adopted worktree rows under the parent project', () => {
    const rows = store.getObservationsByIds([obsId('Worktree finding')], { project: 'acme-app' });
    expect(rows.map(r => r.title)).toEqual(['Worktree finding']);
  });

  it('getObservationsByIds still excludes a row from an unrelated project', () => {
    const rows = store.getObservationsByIds([obsId('Foreign finding')], { project: 'acme-app' });
