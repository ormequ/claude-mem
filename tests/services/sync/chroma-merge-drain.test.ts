...
function fakeChromaSync(failForProject?: string) {
  ...
  return {
    ...
    async updateMergedIntoProject(
      targets: Array<{ docType: string; sqliteId: number }>,
      project: string
    ) {
      calls.push({
        ids: targets.map(t => t.sqliteId),
        project,
...
describe('drainChromaMergeQueue', () => {
  ...
  it('patches dirty observation and summary rows and stamps them on success', async () => {
    const o1 = insertObs(db, 'o1', { merged: 'acme-app' });
    const o2 = insertObs(db, 'o2', { merged: 'acme-app' });
    const s1 = insertSummary(db, { merged: 'acme-app' });
    ...
    expect(obsCall?.project).toBe('acme-app');
  ...
  it('groups by merged_into_project', async () => {
    const a = insertObs(db, 'a', { merged: 'acme-app' });
  ...
  it('is a no-op when the queue is empty', async () => {
    ...
    insertObs(db, 'patched', { merged: 'acme-app', flag: 123 });
  ...
  it('backfills pre-existing merged rows whose flag defaulted to NULL', async () => {
    ...
    const legacy = insertObs(db, 'legacy', { merged: 'acme-app', flag: null });
