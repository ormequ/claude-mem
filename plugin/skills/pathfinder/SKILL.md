...
When searching observations via `search` or `timeline`, translate the query into
English. The store uses an English-only embedding model — a non-English query
returns confident but unrelated results. Keep identifiers (file paths, service
names, ticket ids, error strings) verbatim; translate only the prose around them.
Other languages are fine for literal matches, just not for the semantic part.
