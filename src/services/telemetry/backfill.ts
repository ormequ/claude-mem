...
import { notMachineRelaySql } from '../../shared/user-prompts.js';
...
export function collectDailyRollups(
  ...
  try {
    ...
    const rows = db
      .query(
        `SELECT ${f.day} AS day, COUNT(*) AS c FROM user_prompts WHERE ${f.where} AND ${notMachineRelaySql('user_prompts')} GROUP BY day`
      )
      .all(...params) as Array<{ day: string; c: number }>;
