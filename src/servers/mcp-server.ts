...
**Why:** 10x token savings. Never fetch full details without filtering first.`
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Search query. MUST be in English — non-English queries return unrelated results. Keep identifiers (paths, ticket ids, error strings) verbatim.' },
        limit: { type: 'number', description: 'Max results (default 20)' },
        project: { type: 'string', description: 'Filter by project name' },
        platformSource: { type: 'string', description: 'Accepted for compatibility but IGNORED on reads: worker search is cross-harness, so results span every platform for the project regardless of this value.' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        query: { type: 'string', description: 'Query to find anchor automatically. MUST be in English — non-English queries return unrelated results. Keep identifiers (paths, ticket ids, error strings) verbatim.' },
  ...
  {
    // fork: search truncates prompt_text to ~60 chars and there was no way to read one in
    // full. Anything that classifies prompts was reading heads. See FORK_NOTES.
    name: 'get_prompts',
    description: 'Fetch full prompt text for specific prompt IDs. Search results truncate prompts to ~60 characters; use this to read them in full. Params: ids (array of prompt IDs, required), orderBy, limit, project',
    inputSchema: {
      type: 'object',
      properties: {
        ids: {
          type: 'array',
          items: { type: 'number' },
          description: 'Array of prompt IDs to fetch (the numeric part of #P1234), required'
        }
      },
      required: ['ids'],
      additionalProperties: true
    ...
    handler: async (args: any) => {
      return await callWorker('/api/prompts/batch', { body: args });
    }
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        platformSource: { type: 'string', description: 'Accepted for compatibility but IGNORED: session-start context is cross-harness and spans every platform for the project.' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        query: { type: 'string', description: 'Semantic search query. MUST be in English — non-English queries return unrelated results. Keep identifiers (paths, ticket ids, error strings) verbatim.' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        question: { type: 'string', description: 'Question to ask. MUST be in English — non-English questions return unrelated results. Keep identifiers (paths, ticket ids, error strings) verbatim.' }
      },
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  // FORK: advertise the runtime-visible subset of the smart-tools-gated list,
  // so CLAUDE_MEM_SMART_TOOLS=false still removes them from ListTools.
  const advertisedTools = getAdvertisedMcpToolsForRuntime(enabledTools, selectRuntime());
  return {
