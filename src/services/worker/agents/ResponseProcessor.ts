...
import { getPromptVersionTag, buildObservationMetadata } from './prompt-version.js';
...
export async function processAgentResponse(
  ...
  const promptVersion = getPromptVersionTag();
  const labeledObservations = sanitizedObservations.map(obs => ({
    ...
    metadata: buildObservationMetadata(promptVersion)
  }));
