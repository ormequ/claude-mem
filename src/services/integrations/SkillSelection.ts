...
export const DEFAULT_CLAUDE_MEM_SKILLS = [
  ...
  'pathfinder',
  'knowledge-agent',
  // Fork-owned. In every set: a skill that is not installed cannot be invoked at all,
  // and this one keeps the state the whole review loop is built on
  // (`~/.claude-mem/state/harness-ledger.md`, `harness-review-series.md`).
  'harness-review',
] as const;
...
export const COMPACT_CLAUDE_MEM_SKILLS = [
  ...
  'pathfinder',
  'timeline-report',
  // Fork-owned, and in `compact` despite being periodic rather than a permanent slot:
  // it owns the fix ledger and the run series, so leaving it out of the smallest set
  // means the one loop that measures whether the harness improves is the first thing
  // dropped when trimming.
  'harness-review',
] as const;
