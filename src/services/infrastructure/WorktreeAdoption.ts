...
export async function adoptMergedWorktrees(opts: {
  ...
  try {
    ...
    const clearObsChromaFlag = obsHasFlag
      ? db.prepare('UPDATE observations SET chroma_merge_synced_at = NULL WHERE project = ? AND merged_into_project = ?')
      : null;
    const clearSumChromaFlag = sumHasFlag
      ? db.prepare('UPDATE session_summaries SET chroma_merge_synced_at = NULL WHERE project = ? AND merged_into_project = ?')
      : null;
    ...
    const adoptWorktreeInTransaction = (wt: WorktreeEntry) => {
      ...
      if (syncLane) {
        ...
        // FORK: emitRemapProject sets merged_into_project through the sync
        // outbox, which knows nothing about chroma_merge_synced_at — so the
        // rows it remapped would never enter the worker's ChromaMergeDrain
        // queue. Re-null the flag on exactly those rows (they now carry the
        // parent pointer, so the plain-UPDATE predicate no longer matches).
        clearObsChromaFlag?.run(worktreeProject, parentProject);
        clearSumChromaFlag?.run(worktreeProject, parentProject);
      } else {
