...
import { isMachineRelayPrompt } from '../../../../shared/user-prompts.js';
...
export class ChromaSearchStrategy {
  ...
  private async executeChromaSearch(
    ...
    if (categorized.promptIds.length > 0) {
      // Chroma indexes machine relays too; search means "what the user typed",
      // so drop them here. Fetch-by-id (DataRoutes) still returns them.
      prompts = this.sessionStore.getUserPromptsByIds(categorized.promptIds, {
        orderBy: sqlOrderBy,
        limit: options.limit,
        project: options.project,
        platformSource: options.platformSource
      }).filter(prompt => !isMachineRelayPrompt(prompt.prompt_text));
    }
