...
async function buildHooks() {
  ...
  try {
    ...
    const pluginPackageJson = {
      ...
      type: 'module',
      // Fork delta: no tree-sitter. `tree-sitter-cli` was the only trusted dep,
      // i.e. the only one Bun ran an install script for, and that script downloads
      // a binary from the GitHub release CDN — which this network blocks, so every
      // `bun install` in the marketplace failed and aborted the sync before it
      // filtered skills. The 25 grammars are ~495 MB of dead weight without the
      // CLI. codegraph replaced smart_search/smart_outline here. See FORK_NOTES.
      dependencies: {
        'zod': '^4.4.3',
        'shell-quote': '^1.8.3',
      },
    ...
    if (fs.existsSync('src/integrations/opencode-plugin/index.ts')) {
      ...
      await build({
        entryPoints: ['src/integrations/opencode-plugin/runtime.ts'],
        bundle: true,
        platform: 'node',
        target: 'node18',
        format: 'esm',
