...
`FORK_NOTES.md` (an index over `fork-notes/`) is the authority on *why* these
defaults differ; this file is the *how*. If the two disagree, FORK_NOTES wins
...
deliberately omits and why (see `fork-notes/install-defaults.md`): `learn-codebase`
...
dropped the tree-sitter work entirely (`fork-notes/dropped-tree-sitter.md`).
...
`fork-notes/observations.md`, `src/sdk/parser.ts`). Expect to verify, not assume, on a new
endpoint.

## 5. Verify — do not skip

### 5a. Check the PIN first

Everything below inspects the fork's own directories — but Claude Code decides
what it loads from `~/.claude/plugins/installed_plugins.json`, and it installs
from the marketplace repo's DEFAULT branch. Since 2026-08-15 that is `main`,
which carries the fork (the upstream mirror moved to the `upstream` branch); if
the default branch is ever pointed back at a plain-upstream branch, the next
plugin refresh pins the install to an upstream cache dir and every check below
passes while the running plugin is upstream (`fork-notes/branch-layout.md`).

```bash
python3 -c "import json;p=json.load(open('$HOME/.claude/plugins/installed_plugins.json'))['plugins']['claude-mem@ormequ'][0]['installPath'];print(p);print(open(p+'/.claude-plugin/plugin.json').read()[:200])"
```

The version must contain `+fork.worker.`. If it does not, mirror `plugin/` into
that pinned dir, run `bun install` there, re-filter its `skills/`, and restart
the worker.

### 5b. Check the installed skills directory

Check the directory, not the build log. A failed `bun install`
