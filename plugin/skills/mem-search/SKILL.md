...
## Query in English, always

**Translate the question into English before searching, whatever language the user asked in.**
The store is embedded with an English-only model, so a non-English query lands in a degenerate
region where texts are similar by script rather than by meaning: it returns confident,
plausible-looking results that have nothing to do with the question, and it will not find
English records even when they are exactly on target.

Keep identifiers verbatim — service names, file paths, ticket ids, error strings. Translate
only the prose around them.

- User asks "как проверяется здоровье воркеров" → `search(query="worker health check")`
- User asks "почему падает auth-service при деплое" → `search(query="auth-service crash during deploy")`
