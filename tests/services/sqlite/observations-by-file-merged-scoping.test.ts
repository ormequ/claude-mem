...
describe('getObservationsByFilePath — merged_into_project scoping (Bug 1)', () => {
  ...
  const filePath = 'app/Services/Billing/InvoiceImporter.php';
  ...
  it('surfaces an adopted worktree observation when scoped to the parent project', () => {
    const id = seed('acme-app/bitrix24-deferred-absences', 'wt');
    adopt('acme-app/bitrix24-deferred-absences', 'acme-app');

    const matches = getObservationsByFilePath(store.db, filePath, { projects: ['acme-app'] });
    expect(matches.map(o => o.id)).toContain(id);
  });

  it('still matches the composite slug directly (provenance preserved)', () => {
    const id = seed('acme-app/bitrix24-deferred-absences', 'wt2');
    adopt('acme-app/bitrix24-deferred-absences', 'acme-app');

    const direct = getObservationsByFilePath(store.db, filePath, {
      projects: ['acme-app/bitrix24-deferred-absences'],
    });
